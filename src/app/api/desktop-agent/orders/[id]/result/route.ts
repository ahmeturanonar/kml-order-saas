...
export async function POST(
  ...
  try {
    ...
    const storedFileName = `${Date.now()}-${originalBaseName || "order-result"}-${crypto.randomUUID()}.csv`;
    const originalFileName = `${originalBaseName || order.orderNumber.replace(/[^a-zA-Z0-9-_]+/g, "-") || "order-result"}.csv`;
      ...
      if (transitionedToCompleted) {
        await tx.orderStatusHistory.create({
          data: {
            ...
            note: "CSV generated by desktop agent and uploaded to the server.",
          },
        });
      }
    ...
    if (
      ...
      await deleteStoredFile(previousGeneratedFilePath).catch((error) => {
        logger.warn(
          ...
          "Failed to remove previous generated CSV after replacement",
        );
      });
    }
    ...
    if (transitionedToCompleted) {
      const emailTemplate = orderEmailTemplate({
        ...
        message: `Your order ${updatedOrder.orderNumber} has been completed. Your CSV result file is now ready to download from your dashboard.`,
      ...
      await createNotification({
        ...
        title: `Order ${updatedOrder.orderNumber} completed`,
        message: `Your order ${updatedOrder.orderNumber} has been completed. Your CSV result file is ready to download.`,
